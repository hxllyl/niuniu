...
class Api::PostsController < Api::BaseController
  ...
  def update_all
    if @user.could_update_my_resources?
      ...
      posts.update_all(updated_at: Time.now, expired_at: 7.days.since)
  ...
  #   error_msg: 错误信息
  def filter_brand
    hunts = Post.needs.uncompleted.valid.includes(:user, :brand).where("brands.name" => String(params[:brand_name])).order('posts.created_at desc').page(params[:page] || 1).per(10)
